...
const deleteStory  =asyncErrorWrapper(async(req,res,next)=>{
    ...
    // Check if user is admin or the author of the story
    if (req.user.role !== 'admin' && story.author.toString() !== req.user._id.toString()) {
        return next(new CustomError("You are not authorized to delete this story", 403))
    }
